...
def write_seg(workbook, name, node, *args):
    if node.text is None or node.text.strip() == "" or node.text.isnumeric():
        return ""  # exclude numbers and blanks
    if node.tag not in tag_filter:
    ...
    if "name" in node.attrib:
        ...
        name = ""
    workbook.append([node.text, "", name])
    return name
...
def patch_seg(workbook, name, node, *args):
    ...
    curcell = next(xls_it)
    if node.text == curcell[0].value:
        if curcell[1].value is not None:  # 원문이랑 같지 않다면 패치
        ...
        if fnmatch.fnmatch(file, readpattern):
            ...
            try:
                if xlsdir is None:
                    ...
                    global xls_it
                    xls_cnt = 2
                    xls = xl.load_workbook(
                        filename=f"./{xlsdir}/{file}.xlsx",
                        read_only=True if func == "patch_seg" else False,
                        data_only=True,
                    )
                ...
                if func == "patch_seg":
                    xml.write(
                        f"./{writedir}/{file}", encoding="utf-8", xml_declaration=False
                    )
                else:
